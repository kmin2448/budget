          ...
          <Table className="table-fixed">
            <TableHeader>
              <TableRow className="bg-primary hover:bg-primary">
                <TableHead className="w-12 text-white px-2" />
                <TableHead className={cn("text-white transition-all", collapsed ? "w-[562px]" : "w-[402px]")}>
                  구분 / 프로그램명
                </TableHead>
                <TableHead className="w-16 text-white">소관</TableHead>
                <TableHead className="w-32 text-white">담당교원</TableHead>
                <TableHead className="text-white">담당직원</TableHead>
                <TableHead className="w-24 text-right text-white">예산계획</TableHead>
                <TableHead className="w-24 text-right text-white">집행완료</TableHead>
                <TableHead className="w-24 text-right text-white">집행예정</TableHead>
                <TableHead className="w-16 text-right text-white">집행률</TableHead>
                {canWrite && <TableHead className="w-16 text-center text-white">관리</TableHead>}
              </TableRow>
            </TableHeader>
                ...
                <SortableTbody key={`cat-${key}`} id={key}>
                  {({ attributes, listeners }) => (
                    <>
                      {/* ── 구분 헤더 행 ── */}
                      <TableRow
                        className="cursor-pointer bg-gray-100/70 hover:bg-gray-200"
                      ...
                      >
                        <TableCell className="py-2 pl-2 pr-1">
                          <div className="flex items-center gap-0.5">
                            <div
                              {...attributes}
                              {...listeners}
                              className="cursor-grab active:cursor-grabbing rounded flex-shrink-0 p-1 text-gray-400 hover:bg-gray-200 hover:text-gray-600"
                              onClick={(e) => e.stopPropagation()}
                            >
                        ...
                        <TableCell className="py-2 font-bold text-gray-700 overflow-hidden" colSpan={4}>
                  ...
                  <TableCell className="py-2 text-right font-bold text-gray-700 tabular-nums">
                    {formatKRW(catTotal.budgetPlan)}
                  </TableCell>
                  <TableCell className="py-2 text-right font-bold text-complete tabular-nums">
                    {formatKRW(catTotal.executionComplete)}
                  </TableCell>
                  <TableCell className="py-2 text-right font-bold text-planned tabular-nums">
                  ...
                  groupRows.map((row) => {
                    ...
                    return (
                      <React.Fragment key={`row-group-${row.rowIndex}`}>
                        ...
                        <TableRow
                          className={cn(
                            'transition-colors bg-white hover:bg-gray-50',
                            ...
                            <TableCell className="py-2 text-center">
                              <div className="flex justify-center gap-1">
                                <button
                                  ...
                                  title="수정"
                                  className="rounded p-1.5 text-primary hover:bg-primary-bg"
                                >
                        ...
                        {/* 상세 펼침 행 */}
                        {isRowOpen && hasDetail && (
                          <TableRow className="bg-row-even">
                            <TableCell colSpan={colSpan} className="pb-4 pt-2 pl-[52px] pr-6">
                              <div className="text-sm space-y-2">
                                {/* 프로그램명 (풀 텍스트) */}
                                <div className="pb-2 border-b border-gray-200 flex gap-2">
                                  <span className="font-medium text-gray-500 shrink-0">프로그램명</span>
                                  <span className="text-gray-700 whitespace-pre-wrap flex-1">{getVal(row, 'programName')}</span>
                                </div>
                                {/* 비고 */}
                                {(getVal(row, 'note') || editMode) && (
                                  <div className="pb-2 border-b border-gray-200 flex gap-2">
                                    <span className="font-medium text-gray-500 shrink-0">비고</span>
                                    <InlineEditCell
                                ...
                                <div className="flex flex-wrap gap-4 items-center">
                                  <div className="flex items-center gap-2">
                                    <span className="font-medium text-gray-500">비목</span>
                                    {editMode ? (
                                      <InlineEditCell
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="font-medium text-gray-500">세목</span>
                                    <InlineEditCell
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="font-medium text-gray-500">보조세목</span>
                                    <InlineEditCell
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="font-medium text-gray-500">잔액</span>
                                    <span className={cn(
                                      'font-medium tabular-nums text-gray-500',
                                ...
                                <div className="pt-2 border-t border-gray-200 mt-2">
                                  <div className="font-medium text-gray-500 mb-1">추가 반영사항</div>
                                  <textarea
                                    ...
                                    className="w-full resize-y rounded border border-gray-300 p-2 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary disabled:bg-gray-50 disabled:text-gray-500 disabled:border-transparent"
                                    rows={2}
                                  />
                                </div>
                              </div>
