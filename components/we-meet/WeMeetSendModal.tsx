...
const LS_CATEGORY    = 'wemeet_send_category';
const LS_SUB_CAT     = 'wemeet_send_sub_category';
const LS_SUB_DETAIL  = 'wemeet_send_sub_detail';

interface Props {
  open: boolean;
  row: WeMeetExecution | null;
  onClose: () => void;
}
...
export function WeMeetSendModal({ open, row, onClose }: Props) {
  ...
  useEffect(() => {
    if (open && row) {
      ...
      // 확정금액을 현재 월(인덱스 기준)에 기본 배분
      const currentMonth = new Date().getMonth(); // 0=Jan
      // 회계연도: 3월(idx=0)~2월(idx=11). currentMonth 0=Jan→idx=10, 1=Feb→idx=11, 2=Mar→idx=0...
