...
interface Props {
  ...
  teams: string[];
  usageTypes: string[];
  initialData?: SmallClubExecution;
  defaultTeam?: string;
  onClose: () => void;
  onSubmit: (payload: SmallClubExecutionPayload) => Promise<void>;
  isPending: boolean;
}

const DEFAULT_FORM: SmallClubExecutionPayload = {
  usageType: '',
  description: '',
  teamName: '',
  draftAmount: 0,
...
export function SmallClubRowForm({ open, mode, teams, usageTypes, initialData, defaultTeam, onClose, onSubmit, isPending }: Props) {
  const [form,         setForm]         = useState<SmallClubExecutionPayload>(DEFAULT_FORM);
  ...
  async function handleSubmit() {
    ...
    if (!form.teamName)  { setError('소학회명을 선택해주세요.'); return; }
    setError('');
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="sc-claimed" className={`cursor-pointer ${!isConfirmed ? 'text-gray-400' : ''}`}>
                청구완료
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <input
                id="sc-evidenceSubmitted"
                type="checkbox"
                checked={form.evidenceSubmitted}
                onChange={(e) => setForm((f) => ({ ...f, evidenceSubmitted: e.target.checked }))}
                className="h-4 w-4 accent-primary"
              />
              <Label htmlFor="sc-evidenceSubmitted" className="cursor-pointer">증빙제출</Label>
            </div>
          </div>
          {!isConfirmed && (
