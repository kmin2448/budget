...
import { loadSmallClubSendSettings, saveSmallClubSendSettings } from './SmallClubSendSettingsModal';
...
const LAST_PROGRAM_KEY = 'smallclub_send_last_program';
...
export function SmallClubGroupSendModal({ open, group, initialSelectedIndexes, onClose, onSent }: Props) {
  const router = useRouter();

  const [budgetType, setBudgetType] = useState<'main' | 'carryover'>(() => loadSmallClubSendSettings().budgetType);
  const [category, setCategory]     = useState(() => loadSmallClubSendSettings().category);
  ...
  useEffect(() => {
    saveSmallClubSendSettings({ budgetType, category });
  }, [budgetType, category]);

  useEffect(() => {
    if (open && group) {
      const s = loadSmallClubSendSettings();
  ...
  async function handleSubmit() {
    ...
    if (selectedRows.length === 0)  { setError('소학회를 1개 이상 선택해주세요.'); return; }
    ...
    try {
      ...
      await fetch('/api/small-club/executions/mark-sent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          rowIndexes,
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-medium text-[#131310]">
                  소학회 선택 <span className="text-gray-400 font-normal">({selectedIdxSet.size}/{group.rows.length}개 선택됨)</span>
                </label>
                <button
                  onClick={toggleAll}
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <label className="text-xs font-medium text-[#6F6F6B]">
                ...
                <span className="ml-1 text-[10px] text-gray-400 font-normal">(소학회 선택에 따라 자동 생성)</span>
              </label>
              <input
                type="text"
                value={description}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-medium text-[#6F6F6B]">
                  월별 집행금액
                  <span className="ml-1 text-[10px] text-gray-400 font-normal">(소학회 선택에 따라 자동 배분)</span>
                </label>
        ...
        <DialogFooter>
          {success ? (
            ...
            <>
              ...
              >
                {isPending ? '전송 중…' : `집행내역 전송 (${selectedIdxSet.size}개)`}
              </Button>
            </>
          )}
        </DialogFooter>
