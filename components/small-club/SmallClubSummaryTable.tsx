...
import { SmallClubTeamPdfReport } from '@/components/small-club/SmallClubPdfReport';
import type { SmallClubTeamSummary, SmallClubUsageSummary, SmallClubTeamInfo, SmallClubExecution } from '@/types';

const NO_ADVISOR = '지도교수 미배정';
const ADVISOR_ORDER_KEY = 'smallclub_advisor_order_v1';
...
function calcUsageBreakdown(execs: SmallClubExecution[]): Array<{
...
interface Props {
  ...
  onUpdateTeamInfo: (info: SmallClubTeamInfo) => void;
  onAddExecution: (teamName: string) => void;
  onEditExecution: (row: SmallClubExecution) => void;
  onDeleteExecution: (row: SmallClubExecution) => void;
  onUpdateExecution: (row: SmallClubExecution) => void;
  isToggling: boolean;
  onAdvisorOrderChange?: (order: string[]) => void;
}

function usageTotals(s: SmallClubTeamSummary): { draft: number; confirmed: number; claimed: number } {
  const keys = ['mentoring', 'meeting', 'material', 'studentActivity'] as const;
  return keys.reduce(
    (acc, k) => {
      const u = s[k] as SmallClubUsageSummary;
...
function sumGroups(list: SmallClubTeamSummary[]) {
  return list.reduce(
    (acc, s) => {
...
function toEdit(info: SmallClubTeamInfo): EditState {
  return {
...
export function SmallClubSummaryTable({
  ...
  const executionsByTeam = useMemo(() => {
    const m = new Map<string, SmallClubExecution[]>();
  ...
  const advisorGroups = useMemo(() => {
    const map = new Map<string, SmallClubTeamSummary[]>();
  ...
  function startEdit(info: SmallClubTeamInfo) {
  ...
  async function saveEdit(info: SmallClubTeamInfo) {
    if (!editState) return;
    setSavePending(true);
    try {
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-[#F3F3EE]">
                  <th className="px-3 py-2.5 text-left font-medium text-[#6F6F6B]">소학회명</th>
                ...
                <SortableAdvisorBody key={advisor} id={advisor}>
                  {(dragHandle) => {
                    ...
                    return [
                      ...
                      ...(!isAdvisorOpen
                        ? []
                        : teams.flatMap((s: SmallClubTeamSummary, idx: number) => {
                                          ...
                                          {info ? (
                                            <div className="space-y-3">
                                              <div className="flex items-center justify-between">
                                                <p className="text-xs font-semibold text-primary">{info.teamName} — 소학회 정보</p>
                                                <div className="flex items-center gap-1.5">
                                                  <SmallClubTeamPdfReport
                                                    teamName={s.teamName}
                                                    summary={s}
                                            ...
                                            <p className="text-xs text-gray-400">
                                              소학회 정보가 등록되어 있지 않습니다. 소학회 관리에서 소학회 정보를 입력해 주세요.
                                            </p>
                                          )}
              ...
              <tfoot>
                <tr className="border-t-2 border-[#E3E3E0] bg-[#F3F3EE] font-semibold">
                  <td className="px-3 py-2 text-[#6F6F6B]">합계 ({summaries.length}개)</td>
