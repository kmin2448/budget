...
import type { SmallClubExecutionPayload } from '@/hooks/useSmallClub';
...
interface Props {
  ...
  usageTypes: string[];
  onClose: () => void;
  onSave: (payloads: SmallClubExecutionPayload[]) => Promise<void>;
  isPending: boolean;
}
...
export function SmallClubBulkAddModal({ open, teams, usageTypes, onClose, onSave, isPending }: Props) {
  ...
  function loadAllTeams() {
    ...
    setRows((prev) => {
      const base = prev.filter((r) => r.teamName);
  ...
  async function handleSave() {
    ...
    if (valid.length === 0) { setError('최소 1개 소학회를 입력해주세요.'); return; }
    setError('');

    const payloads: SmallClubExecutionPayload[] = valid.map((r) => {
      ...
      <DialogContent className="!w-[min(1000px,calc(100vw-2rem))] !max-w-[min(1000px,calc(100vw-2rem))] rounded-[2px]" showCloseButton={false}>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 rounded-md border border-[#E3E3E0] bg-[#F8FAFC] px-3 py-2">
            ...
            <input type="text" value={bulkRemarks} onChange={(e) => setBulkRemarks(e.target.value)} placeholder="비고"
            ...
            <input type="text" value={bulkDraftStr}
              onChange={(e) => { const raw = e.target.value.replace(/[^0-9]/g, ''); setBulkDraftStr(raw === '' ? '' : Number(raw).toLocaleString('ko-KR')); }}
              placeholder="기안금액"
            ...
            <input type="text" value={bulkConfStr}
              onChange={(e) => { const raw = e.target.value.replace(/[^0-9]/g, ''); setBulkConfStr(raw === '' ? '' : Number(raw).toLocaleString('ko-KR')); }}
              placeholder="확정금액"
              className="w-28 rounded border border-[#E3E3E0] bg-white px-2 py-1 text-right text-xs focus:outline-none focus:ring-1 focus:ring-primary" />
            <button onClick={applyBulk} disabled={!bulkRemarks && !bulkDraftStr && !bulkConfStr}
              ...
              <ChevronsDown className="h-3 w-3" />전체 행에 적용
            </button>
          </div>
            ...
            <table className="w-full border-collapse text-xs">
              <thead className="sticky top-0 z-10">
                <tr className="bg-[#F3F3EE]">
                  <th className="px-3 py-2 text-left font-medium text-[#6F6F6B] whitespace-nowrap">소학회명 <span className="text-red-400">*</span></th>
                  ...
                  return (
                    <tr key={row.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-[#F5F9FC]'}>
                      <td className="px-2 py-1.5 min-w-[130px]">
                        <select value={row.teamName} onChange={(e) => updateRow(row.id, { teamName: e.target.value })} className={fis}>
                          <option value="">소학회 선택</option>
                          {teams.map((t) => <option key={t} value={t}>{t}</option>)}
                        </select>
                      </td>
                      <td className="px-2 py-1.5">
                        <input type="text" value={row.remarks} onChange={(e) => updateRow(row.id, { remarks: e.target.value })} placeholder="비고" className={fi} />
                      </td>
                      <td className="px-2 py-1.5 min-w-[100px]">
                        <input type="text" value={row.draftStr}
                          onChange={(e) => { const raw = e.target.value.replace(/[^0-9]/g, ''); updateRow(row.id, { draftStr: raw === '' ? '' : Number(raw).toLocaleString('ko-KR') }); }}
                          placeholder="0" className={`${fi} text-right`} />
                      </td>
                      <td className="px-2 py-1.5 min-w-[100px]">
                        <input type="text" value={row.confirmedStr}
                          onChange={(e) => {
                            const raw = e.target.value.replace(/[^0-9]/g, '');
                            const fmt = raw === '' ? '' : Number(raw).toLocaleString('ko-KR');
                            updateRow(row.id, { confirmedStr: fmt, claimed: Number(raw) === 0 ? false : row.claimed });
                          }}
                          placeholder="0=미확정" className={`${fi} text-right`} />
                      </td>
                      <td className="px-2 py-1.5 text-center">
                        <input type="checkbox" checked={row.claimed} disabled={confAmt === 0}
              ...
              {validCount > 0 && (
                <tfoot>
                  <tr className="border-t border-[#E3E3E0] bg-[#F3F3EE]">
                    <td colSpan={2} className="px-3 py-1.5 text-[11px] text-[#6F6F6B]">합계 ({validCount}개)</td>
          ...
          <div className="flex items-center gap-2">
            <button onClick={addRow} className="flex items-center gap-1 text-xs text-gray-500 hover:text-primary transition-colors">
              <Plus className="h-3 w-3" />행 추가
            </button>
            <span className="text-gray-200">|</span>
            <button onClick={loadAllTeams} disabled={teams.length === 0}
              className="flex items-center gap-1 text-xs text-gray-500 hover:text-primary transition-colors disabled:opacity-40">
              <Plus className="h-3 w-3" />전체 소학회 불러오기
              {teams.length > 0 && (
                <span className="ml-0.5 rounded-full bg-gray-100 px-1.5 py-px text-[10px] text-gray-500">{teams.length}개</span>
              )}
            </button>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => { void handleSave(); }} disabled={isPending || validCount === 0 || !usageType || !description}>
            {isPending ? '저장 중...' : `저장 (${validCount}개 소학회)`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
