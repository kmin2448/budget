...
import { upsertSmallClubTeamInfo, deleteSmallClubTeamInfo } from '@/lib/google/smallclub-sheets';
...
export async function PATCH(
  ...
  try {
    ...
    await upsertSmallClubTeamInfo(body, rowIndex);
    return NextResponse.json({ ok: true });
  } catch (err) {
    const message = err instanceof Error ? err.message : '소학회 정보 수정 실패';
...
export async function DELETE(
  ...
  try {
    await assertCanWrite(session.user.email);
    await deleteSmallClubTeamInfo(rowIndex);
    return NextResponse.json({ ok: true });
  } catch (err) {
    const message = err instanceof Error ? err.message : '소학회 정보 삭제 실패';
    const status = message === '권한이 없습니다.' ? 403 : 500;
    return NextResponse.json({ error: message }, { status });
  }
}
